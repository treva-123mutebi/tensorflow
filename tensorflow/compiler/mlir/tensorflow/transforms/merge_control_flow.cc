...
// For 2D vector defined below, the first dimension represents the class of the
// IfRegion group and the second dimension represents the segments of the
// IfRegion group. For example, if we want to merge the following six IfRegions
...
// group. Then when we call FindSubgroupLastIfOpIter for the first group, it
...
IfOpIterConst FindSubgroupLastIfOpIter(
    IfOpIterConst current, IfOpIterConst potential_last,
...
// Return a vector that contains the ops to be moved after merged IfRegion for
// a segment of IfRegions, the ops are in the sequential order
...
void GenerateSegmentsPerIfGroups(
  ...
  auto it2 = moved_ops_groups.try_emplace(if_cond);
...
// Update the moved op list before IfRegions to be erased
void UpdateMovedOpList(llvm::SmallVector<TF::IfRegionOp, 8>& if_op_segment,
                       MapToOperationVec2D& moved_ops_groups,
                       TF::IfRegionOp new_if_op) {
  Block* block = new_if_op.getOperation()->getBlock();
  auto if_cond = new_if_op.cond();
  for (auto& entry : moved_ops_groups) {
    for (llvm::SmallVector<mlir::Operation*, 8>& moved_op_ordered :
         entry.second) {
      ...
      for (auto& if_op : if_op_segment) {
        auto iter = std::find(moved_op_ordered.begin(), moved_op_ordered.end(),
                              if_op.getOperation());
        if (iter != moved_op_ordered.end()) {
          moved_op_ordered.erase(iter);
          needAddNewIfOp = true;
        }
      }
      if (needAddNewIfOp) {
        moved_op_ordered.push_back(new_if_op.getOperation());
        absl::flat_hash_set<Operation*> all_moved_ops(moved_op_ordered.begin(),
                                                      moved_op_ordered.end());
        moved_op_ordered.clear();
        for (Operation& op : *block) {
          if (all_moved_ops.count(&op)) {
            moved_op_ordered.push_back(&op);
          }
        }
      }
    }
...
void MergeIfPerSegment(llvm::SmallVector<TF::IfRegionOp, 8>& if_op_segment,
  ...
  UpdateMovedOpList(if_op_segment, moved_ops_groups, new_if_op);
...
// Merge IfRegions for each IfRegion group. Each IfRegion group contain
...
void MergeIfPerIfGroups(const Value& if_cond,
  ...
  for (auto i = 0; i < segments.size(); ++i) {
    if (segments[i].size() >= 2) {
      MergeIfPerSegment(segments[i], moved_ops_group[i], moved_ops_groups);
    }
  }
}
