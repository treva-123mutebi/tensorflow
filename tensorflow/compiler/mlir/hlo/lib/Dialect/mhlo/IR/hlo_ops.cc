...
LogicalResult BitcastConvertOp::verify() {
  ...
  auto target_shape_prefix = target_shape.drop_back(1);
  auto operand_shape_prefix = operand_shape.drop_back(1);
  if (target_shape_prefix != operand_shape_prefix) {
    return emitOpError()
           << "target and operand shapes must match in their non-final "
           << "positions. Got: " << target_shape << " and " << operand_shape
           << ".";
  }
