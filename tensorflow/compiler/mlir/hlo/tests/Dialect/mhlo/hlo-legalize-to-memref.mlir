...
func @dyn_broadcast_unsigned(%operand: tensor<?x?xui32>, %shape: tensor<3xi64>) -> tensor<?x?x?xui32> {
  // CHECK-SAME: %[[ARG:.*]]: tensor<?x?xui32>, %[[SHAPE:.*]]: tensor<3xi64>
  %c1 = arith.constant 1 : i64
  %result = "mhlo.dynamic_broadcast_in_dim"(%operand, %shape) {
    broadcast_dimensions = dense<[1, 2]> : tensor<2xi64>
...
func @dyn_reshape_unsigned(%operand: tensor<?x?xui32>, %shape: tensor<3xi64>) -> tensor<?x?x?xui32> {
  // CHECK-SAME: %[[ARG:.*]]: tensor<?x?xui32>, %[[SHAPE:.*]]: tensor<3xi64>
  %c1 = arith.constant 1 : i64
...
// CHECK-LABEL:     func @reshape_unsigned
// CHECK-DAG:         %[[C0:.*]] = arith.constant 0 : i128
// CHECK-DAG:         %[[DELTA:.*]] = arith.constant 1 : i128
// CHECK-DAG:         %[[C64:.*]] = arith.constant 64 : i128
